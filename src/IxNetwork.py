...
class IxNet:
        ...
        try:
            ...
            if self._socket is None:
                ...
                return "Cannot connect to {0}:{1} as a connection is already established to {2}:{3}. Please execute disconnect before trying this command again.".format(
                    address, int(nameValuePairs['-port']), sockInfo[0], int(sockInfo[1]))
        except (socket.error,):
            e = sys.exc_info()[1]
            self.__Close()
    ...
    def _CheckClientVersion(self):
        if self._version != self.getVersion():
            print("WARNING: IxNetwork Python library version %s is not matching the IxNetwork client version %s" % (
